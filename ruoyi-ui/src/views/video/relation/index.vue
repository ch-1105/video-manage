...
<script>
import { listRelation, getRelation, delRelation, addRelation, updateRelation } from "@/api/video/relation"
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset()
      this.open = true
      this.title = "添加视频与标签关联"
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset()
      const videoId = row.videoId || this.ids
      getRelation(videoId).then(response => {
        this.form = response.data
        this.open = true
        this.title = "修改视频与标签关联"
      })
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除视频与标签关联编号为"' + videoIds + '"的数据项？').then(function() {
        return delRelation(videoIds)
      }).then(() => {
        this.getList()
